...
try:
    ...
    from hoja.app.tasks import TASKS
...
class HojaEnvironment(Environment):
    ...
    def __init__(self):
        ...
        if self._task not in TASKS:
            self._task = "medium"
        self._cfg = TASKS[self._task]
        self._max_steps = self._cfg.max_steps
        
        self._grader = Grader()
        
        self._total_vehicles_cleared = 0
        self._total_vehicles_arrived = 0
        self._total_emergencies = 0
        self._emergencies_cleared = 0
        self._pedestrian_violations = 0
    ...
    def reset(self) -> HojaObservation:
        ...
        for d in DIRECTIONS:
            ...
            self._total_vehicles_arrived += arrivals
    ...
    def step(self, action: HojaAction) -> HojaObservation:
        ...
        # --- 4. Pedestrians ----------------------------------------
        if self._cfg.pedestrians:
            if self._pedestrian_count > 8:
                self._pedestrian_violations += 1
        ...
        # Calculate rich metrics for Grade report
        throughput_ratio = self._total_vehicles_cleared / max(1, self._total_vehicles_arrived)
        safety_score = max(0.0, 1.0 - (self._pedestrian_violations * 0.1) - (self._emergency_wait * 0.05))
        emergency_cleared_ratio = self._emergencies_cleared / max(1, self._total_emergencies)
        
        metrics = {
            "throughput_ratio": throughput_ratio,
            "safety_score": safety_score,
            "avg_wait": avg_wait,
            "emergency_cleared_ratio": emergency_cleared_ratio
        }

        # Apply grader score to metadata, replace basic reward logic with the true graded score when done!
        grader_report = self._grader.compute_score(metrics)
        step_metadata = {
            "task": self._task,
        ...
        # Instead of just taking the raw bounded reward, let's incorporate the grader score dynamically if needed
        # We can either set `reward` to the grader total score, or keep it as step-based. For now keep step-based,
        # but when `done=True`, inject it.
        final_reward = grader_report["total_score"] if done else round(reward, 2)
    ...
    def _build_observation(
        self, status_message: str, reward: float, done: bool, metadata: dict = None
    ) -> HojaObservation:
        avg_wait = sum(self._wait_times.values()) / len(DIRECTIONS)
